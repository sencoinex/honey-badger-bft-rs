...
use crate::{encode::Coder, node::NodeId, Result};
...
impl<ID: NodeId, IDX: ValidatorIndex> ValidatorSet<ID, IDX> {
    pub fn new(indices: BTreeMap<ID, IDX>) -> Result<Self> {
        ...
        let parity_shards = 2 * max_durable_faulty_size;
        let data_shards = size - parity_shards;
        let encoder = Coder::new(data_shards, parity_shards)?;
        Ok(Self {
            indices,
            encoder,
